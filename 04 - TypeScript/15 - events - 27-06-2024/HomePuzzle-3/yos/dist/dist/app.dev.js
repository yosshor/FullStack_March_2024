...
var animalsArray = getAnimalsArray(); // [dragon, parrot, dog, cat, hippo, dragon1, parrot1, dog1, cat1, hippo1]
...
function writeAnimalsToDom(arr) {
  try {
    var html = '';
    var animals = document.getElementById('animals'); // console.log('sdf', arr)
...
function findElementByIdOrName(id, name) {
  try {
    if (id) return animalsArray.find(function (e) {
      return e.id == id;
    });
  ...
  try {
    setTimeout(function () {
      ...
      if (arr === undefined) {
        ...
        var findElementInArr = arr.findIndex(function (e) {
          return e.id === id;
        });
