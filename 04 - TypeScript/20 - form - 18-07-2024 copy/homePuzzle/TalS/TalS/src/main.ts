...
class Sandwich {
  ...
  constructor(name: string, type: string, size: string, remove: string[]) {
    this.name = name;
    this.type = type;
    this.size = size;
    this.remove = remove;
...
function handleSandwichOrder(ev: any) {
  try {
    ev.preventDefault();
    console.log(ev);
    const form = ev.target;
    const name = form.name.value;
    const type = form.sandwichType.value;
    const size = form.size.value;
    const remove: string[] = [];
    for (let i = 0; i < form.elements.length; i++) {
      const element = form.elements[i];
      if (element.type === "checkbox" && element.checked) {
        remove.push(element.value);
      }
    }
    const sandwich = new Sandwich(name, type, size, remove);
